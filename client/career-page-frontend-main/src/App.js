    ...
    <>
      ...
      <React.Suspense fallback={<Loader />}>
        ...
        <Route path="/*" element={<Layout />}>
          ...
          <Route element={<ProtectedRoute allowedRoles={allowedRoles.concat(["Applicant"])} />}><Route path=":id?/profile" element={<MainForm />} /></Route>
          <Route element={<ProtectedRoute allowedRoles={allowedRoles.concat(["Applicant"])} />}><Route path="application" element={<MyApplication />} /></Route>
          <Route element={<ProtectedRoute2 allowedRoles={allowedRoles.concat(["Recruiter"])} />}><Route path="dashboard" element={<HrDashHomePage />} /></Route>
          
          <Route element={<ProtectedRoute2 allowedRoles={allowedRoles.concat(["Recruiter"])} />}><Route path="applicant/*" element={<AppliedJobs />} /></Route>

          
          <Route element={<ProtectedRoute2 allowedRoles={allowedRoles.concat(["Recruiter"])} />}><Route path="create-job" element={<JdInputFields />} /></Route>
