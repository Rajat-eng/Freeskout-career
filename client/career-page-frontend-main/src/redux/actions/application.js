...
export const submitApplication=(id)=>async(dispatch)=>{
    try {
        ...
        if(data.success){
           ...
           dispatch({type:applicationConstants.SUBMIT_APPLICATION_FAIL,payload:data.message})
        }
      } catch (error) {
        console.log(error);
        dispatch({type:applicationConstants.SUBMIT_APPLICATION_FAIL,payload:error.response.data.message})
    }
}
...
export const getMyApplications=()=>async(dispatch)=>{
  try {
    ...
    if(data.success){
       dispatch({type:applicationConstants.GET_MY_APPLICATIONS_SUCCESS,payload:data.applications})
    }else{
       dispatch({type:applicationConstants.GET_MY_APPLICATIONS_FAIL,payload:data.message})
    }
  } catch (error) {
    console.log(error);
    dispatch({type:applicationConstants.SUBMIT_APPLICATION_FAIL,payload:error.response.data.message})
