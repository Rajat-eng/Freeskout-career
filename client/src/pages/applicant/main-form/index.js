  ...
  async function submit(e) {
    ...
    try {
      ...
      const res = await fetch(
        "https://freeskout-career.onrender.com/api/v1/applicant/upload",
        {
          headers: {
  ...
  return (
    <div className={style.mainForm}>
      ...
      <nav className={style.profileNav}>
        <ul>
          <li
            className={`${page === 1 && style.green}`}
            onClick={() => setPage(1)}
          >
            Basic
          </li>
          <li
            className={`${page === 2 && style.green}`}
            onClick={() => setPage(2)}
          >
            Details
          </li>
          <li
            className={`${page === 3 && style.green}`}
            onClick={() => setPage(3)}
          >
            Edu & Work Exp
          </li>
          <li
            className={`${page === 4 && style.green}`}
            onClick={() => setPage(4)}
          >
            Skills & Resume
          </li>
      ...
      <div className={style.mainForm_container}>
        ...
        {page === 4 ? (
          <div className={style.func_buttons}>
            ...
            <button disabled={loading === true} onClick={submit}>
              Submit
            </button>
          </div>
        ) : (
